...
import { AlertTriangle, ArrowLeft, Building2, ChevronLeft, ChevronRight, CreditCard, Eye, Filter, Loader2, Map, PackageOpen, RefreshCw, Search, Tag, Truck, User, X } from 'lucide-react';
...
import EnRouteWaybillDetailDialog from './delivery/en-route/dialogs/EnRouteWaybillDetailDialog';
import UpdateEnRouteStatusDialog from './delivery/en-route/dialogs/UpdateEnRouteStatusDialog';
import type { BadgeConfig, EnRouteFilters, EnRouteWaybill, EnRouteWaybillDetail, FilterOption, HubSummary, ListResponse, TripSummary, UserSummary } from './delivery/en-route/types';
...
const MANAGER = 32;
const DIRECTOR = 64;
const defaultFilters: EnRouteFilters = { keyword: '', statuses: ['IN_TRANSIT'], driverIds: [], tripIds: [], routeIds: [], originHubIds: [], destHubIds: [], paymentTypes: [], page: 1, limit: 10 };
...
const statusConfig: Record<string, BadgeConfig> = {
  ...
  IN_TRANSIT: { label: 'Đang giao dọc đường', className: 'bg-sky-50 text-sky-700 border-sky-200' },
...
const canUpdateEnRoute = (roleMask: number) => (roleMask & (DRIVER | DISPATCHER | MANAGER | DIRECTOR)) !== 0;
...
const normalizeTotal = (response: ListResponse<EnRouteWaybill> | EnRouteWaybill[], fallback: number) => Array.isArray(response) ? fallback : response.total ?? response.meta?.total ?? fallback;
const displayValue = (value: unknown, suffix = '') => value === null || value === undefined || value === '' ? '—' : `${value}${suffix}`;
const normalizeStatus = (waybill: EnRouteWaybill) => String(waybill.current_state || '').toUpperCase();
const isUpdatable = (waybill: EnRouteWaybill) => normalizeStatus(waybill) === 'IN_TRANSIT';
const getDriverName = (waybill: EnRouteWaybill) => waybill.last_mile_driver?.name || waybill.driver?.name || waybill.last_mile_driver?.username || waybill.driver?.username || (waybill.last_mile_driver_id ? `Tài xế #${waybill.last_mile_driver_id}` : '—');
...
export default function DeliveryEnRoutePage() {
  ...
  const [filters, setFilters] = useState<EnRouteFilters>(defaultFilters);
  const [waybills, setWaybills] = useState<EnRouteWaybill[]>([]);
  ...
  const [detailWaybill, setDetailWaybill] = useState<EnRouteWaybillDetail | null>(null);
  const [statusWaybill, setStatusWaybill] = useState<EnRouteWaybill | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [actionError, setActionError] = useState('');
  const user = useMemo(getStoredUser, []);
  const canUpdate = canUpdateEnRoute(user?.role_mask ?? 0);
  ...
  const updateFilter = <K extends keyof EnRouteFilters>(key: K, value: EnRouteFilters[K]) => setFilters(prev => ({ ...prev, [key]: value, page: key === 'page' ? value as number : 1 }));
  ...
  const loadWaybills = async () => {
    ...
    try {
      const response = await apiRequest<ListResponse<EnRouteWaybill> | EnRouteWaybill[]>(`/waybills?${buildWaybillQuery()}`);
      const items = normalizeList(response, 'waybills');
      setWaybills(items); setTotal(normalizeTotal(response, items.length));
    } catch (err) { setError(err instanceof ApiError ? err.message : 'Không tải được danh sách vận đơn dọc đường.'); }
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      apiRequest<ListResponse<TripSummary> | TripSummary[]>('/trips?status=IN_TRANSIT&page=1&limit=100'),
  ...
  const openDetail = async (waybill: EnRouteWaybill) => {
    try { setDetailWaybill(await apiRequest<EnRouteWaybillDetail>(`/waybills/${waybill.id}`)); }
  ...
  const confirmUpdateStatus = async () => {
    ...
    try {
      await apiRequest(`/waybills/${statusWaybill.id}/status`, { method: 'PATCH', body: { status: 'AT_DEST_HUB' } });
  ...
  return (
    <div className="h-full min-h-0 flex flex-col gap-2">
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden flex-1 min-h-0 flex flex-col">
        <div className="p-3 border-b border-border shrink-0 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button disabled className="flex h-9 items-center gap-2 rounded-lg bg-primary px-3 text-[13px] font-black text-white opacity-50"><RefreshCw size={16} />Thêm</button>
          </div>
        ...
        {isLoading ? <StateBlock icon={<Loader2 className="animate-spin" size={24} />} title="Đang tải vận đơn" description="Hệ thống đang lấy danh sách vận đơn giao dọc đường." /> : error ? <StateBlock icon={<AlertTriangle size={24} />} title="Không tải được dữ liệu" description={error} /> : !waybills.length ? <StateBlock icon={<PackageOpen size={24} />} title="Chưa có vận đơn" description="Không có vận đơn giao dọc đường khớp bộ lọc hiện tại." /> : (
      ...
      <EnRouteWaybillDetailDialog waybill={detailWaybill} onClose={() => setDetailWaybill(null)} formatHub={formatHubById} renderStatus={renderStatus} renderPayment={renderPayment} />
      <UpdateEnRouteStatusDialog waybill={statusWaybill} isSubmitting={isSubmitting} error={actionError} onClose={() => setStatusWaybill(null)} onConfirm={confirmUpdateStatus} />
...
function MobileCard({ waybill, canUpdate, formatHub, openDetail, openUpdate }: { waybill: EnRouteWaybill; canUpdate: boolean; formatHub: (id?: string | number | null) => string; openDetail: (waybill: EnRouteWaybill) => void; openUpdate: () => void }) { return <div className="rounded-2xl border border-border bg-white p-3 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="font-black text-foreground">{waybill.waybill_code}</div><div className="mt-1 flex flex-wrap gap-1">{renderStatus(waybill.current_state)}{renderPayment(waybill.payment_type)}</div></div><div className="flex gap-2"><IconButton title="Xem" onClick={() => openDetail(waybill)}><Eye size={16} /></IconButton><IconButton title="Cập nhật" disabled={!canUpdate} onClick={openUpdate}><Truck size={16} /></IconButton></div></div><div className="mt-3 grid gap-2 text-[13px]"><MobileInfo label="Người gửi" value={waybill.sender_info} /><MobileInfo label="Người nhận" value={waybill.receiver_info} /><MobileInfo label="Hub đi" value={<HubBadge>{formatHub(waybill.origin_hub_id)}</HubBadge>} /><MobileInfo label="Hub đến" value={<HubBadge>{formatHub(waybill.dest_hub_id)}</HubBadge>} /><MobileInfo label="Tài xế" value={<DriverBadge>{getDriverName(waybill)}</DriverBadge>} /><MobileInfo label="Cân nặng" value={displayValue(waybill.weight, ' kg')} /><MobileInfo label="Kích thước" value={`${displayValue(waybill.length)} × ${displayValue(waybill.width)} × ${displayValue(waybill.height)}`} /><MobileInfo label="TL quy đổi" value={displayValue(waybill.volumetric_weight, ' kg')} /><MobileInfo label="Cước phí" value={displayValue(waybill.cost_amount)} /></div></div>; }
