...
export default function WarehouseManifestsPage() {
  ...
  function openAddWaybills(manifest: LoadPlanningManifest) { if (!canManageManifest || manifest.status !== 'DRAFT') return; setAddWaybillsManifest(manifest); setAddWaybillsForm({ keyword: '', selectedIds: [], page: 1, limit: 20 }); setIsAddWaybillsOpen(true); }
  async function fetchWaybillChoices() {
    if (!addWaybillsManifest) return;
    setIsWaybillLoading(true);
    try { const params = new URLSearchParams({ status: 'IN_WAREHOUSE', origin_hub_id: String(addWaybillsManifest.origin_hub_id || ''), dest_hub_id: String(addWaybillsManifest.dest_hub_id || ''), keyword: addWaybillsForm.keyword, page: String(addWaybillsForm.page), limit: String(addWaybillsForm.limit) }); const response = await apiRequest<any>(`/waybills?${params.toString()}`); const list = Array.isArray(response) ? response : response.data || response.items || response.waybills || []; setWaybillChoices(list); setWaybillTotal(Array.isArray(response) ? list.length : response.total ?? response.meta?.total ?? list.length); }
  ...
  async function submitAddWaybills() {
    ...
    setIsSubmitting(true); setActionError('');
    try { await apiRequest(`/manifests/${addWaybillsManifest.id}/waybills`, { method: 'POST', body: { waybill_ids: addWaybillsForm.selectedIds } }); closeAddWaybills(); await fetchManifests(); }
    ...
    <div className="h-full min-h-0 flex flex-col gap-2">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden flex-1 min-h-0 flex flex-col">
        ...
        <div className="flex-1 min-h-0 overflow-auto custom-scrollbar">
          {isLoading ? <StateBlock icon={<Loader2 size={22} className="animate-spin" />} title="Đang tải danh sách bảng kê..." /> : error ? <StateBlock icon={<AlertTriangle size={22} />} title={error} /> : !manifests.length ? <StateBlock icon={<PackageCheck size={22} />} title="Chưa có bảng kê phù hợp." /> : <><table className="hidden md:table w-full min-w-[1280px] text-left border-collapse"><thead className="bg-slate-100 text-[11px] uppercase tracking-wider text-slate-600"><tr>{['Mã bảng kê','Hub đi','Hub đến','Trạng thái','Tổng vận đơn','Tổng trọng lượng','Chuyến xe','Thời gian đóng','Người đóng','Thao tác'].map(header => <th key={header} className={clsx("border-r border-border px-4 py-3 font-bold last:border-r-0", header === "Thao tác" && "w-px whitespace-nowrap")}>{header}</th>)}</tr></thead><tbody>{manifests.map(manifest => <ManifestRow key={manifest.id} manifest={manifest} mayAssign={mayAssign} canManage={canManageManifest} onDetail={openDetail} onAssign={openAssign} onEdit={openEdit} onAddWaybills={openAddWaybills} onCloseManifest={confirmCloseManifest} onPrint={openPrint} onDelete={confirmDeleteManifest} />)}</tbody></table><div className="grid gap-3 p-3 md:hidden">{manifests.map(manifest => <ManifestCard key={manifest.id} manifest={manifest} mayAssign={mayAssign} canManage={canManageManifest} onDetail={openDetail} onAssign={openAssign} onEdit={openEdit} onAddWaybills={openAddWaybills} onCloseManifest={confirmCloseManifest} onPrint={openPrint} onDelete={confirmDeleteManifest} />)}</div></>}
        </div>
      ...
      <AddWaybillsToManifestDialog isOpen={isAddWaybillsOpen} isClosing={isAddWaybillsClosing} isLoading={isWaybillLoading} isSubmitting={isSubmitting} waybills={waybillChoices} total={waybillTotal} formState={addWaybillsForm} onChange={patch => setAddWaybillsForm(prev => ({ ...prev, ...patch }))} onClose={closeAddWaybills} onSubmit={submitAddWaybills} />
...
function Actions({ manifest, mayAssign, canManage, onDetail, onAssign, onEdit, onAddWaybills, onCloseManifest, onPrint, onDelete }: ManifestItemProps) {
  ...
  return (
    <div className="inline-flex flex-nowrap gap-2">
      ...
      <IconAction label="Thêm đơn" disabled={!canManage || !isDraft} onClick={() => onAddWaybills(manifest)} icon={<FilePlus2 size={15} />} />
      <IconAction label="Đóng bảng kê" disabled={!canManage || !isDraft} onClick={() => onCloseManifest(manifest)} icon={<PackageCheck size={15} />} />
      <IconAction label="Gán chuyến" disabled={!mayAssign || !isClosed} onClick={() => onAssign(manifest)} icon={<Route size={15} />} />
